...
import {
  getMyWizardFromId,
  getCampaignFromId,
} from "../helperFuncs/helperFunctions.js";
...
export function CampaignView() {
  ...
  const handleButton = (type, target = null) => {
    switch (type) {
      case "campaign":
        setCurrentCampaignData({
          campaign: target,
          wizard: getMyWizardFromId(target.wizardId, refData),
        });
        break;
      case "back":
        setCurrentCampaignData({ campaign: null, wizard: null });
  ...
  const handleView = (location) => {
    ...
    setCurrentCampaignData({
      ...campaign,
      wizard: getMyWizardFromId(campaign.wizardId, refData),
    });
    ...
    <>
      {!currentCampaignData.campaign && (
        <Box sx={{ width: "100%", textAlign: "center" }}>
          ...
          <p>-Battle Views</p>
          <Button onClick={() => handleButton("newCampaign")}>
            {" "}
            + New Campaign
          </Button>
        </Box>
...
export function CampaignSideDrawer() {
  ...
  const handleButton = (type, target = null) => {
    switch (type) {
      case "campaign":
        setCurrentCampaignData({
          campaign: target,
          wizard: getMyWizardFromId(target, refData),
        });
        break;
      case "wizard":
        break;
  ...
  const handleCampaignDeletion = (campaignId) => {
    ...
    showAlertDialog(confirmText, "This campaign will be gone forever!").then(
      (confirmed) => {
        if (confirmed) {
          // POST request to hire the apprentice
          const newUserData = { ...userData };
          newUserData.myCampaigns = newUserData.myCampaigns.filter(
  ...
  const campaignList = userData.myCampaigns
    ...
    .map((campaign) => (
      <ListItem
        ...
        disableGutters
      >
        <ListItemButton onClick={() => handleButton("campaign", campaign)}>
          <ListItemText
...
function CampaignDetailsView({ handleButton, handleView, campaignView }) {
  ...
  const isAllComplete = currentCampaignData.campaign.scenarios.every(
    (scenario) => scenario.completionStatus === "complete"
  );
    ...
    <>
      <Box sx={{ width: "100%", textAlign: "center" }}>
        ...
        <p>
          Wizard:{" "}
          {
            getMyWizardFromId(currentCampaignData.campaign.wizardId, userData)
              .name
          }
        </p>
      </Box>
      ...
      >
        {currentCampaignData.campaign.scenarios.map((scenario) => (
          <ScenarioCard
            key={scenario.id}
            scenario={scenario}
            handleView={handleView}
      ...
      <Box sx={{ width: "100%", textAlign: "center" }}>
        ...
        <Button
          ...
          onClick={() => handleView("addScenario")}
        >
          Add Scenario
        </Button>
      </Box>
